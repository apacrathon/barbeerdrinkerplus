...
app.controller('myCtrl', [
  ...
  function($scope) {
    ...
    jQuery(document).ready(function(){
      query.find({
        query: {
          rawQuery: "SELECT DISTINCT city, state from test.bars;"
        }
    ...
    jQuery(document).ready(function() {
      $scope.mostPopularClick = function () {
        jQuery('#allCitiesDropDown').removeClass().addClass("modalFormHidden");
        jQuery('#allCitiesDropDown').removeClass().addClass("modalFormShown");
      };
      jQuery('.allCities').on('select2:close',function() {
        ...
        query.find({
          query: {
            rawQuery: "SELECT * FROM test.checkin C, test.bars B WHERE C.barName = B.name AND B.city LIKE " +
            "'%"+inputCity+"%' GROUP BY C.barName ORDER BY Count(*) DESC LIMIT 10;"
          }
        }).then(function(response) {
          $scope.$apply(() => {
            $scope.searchInitiated = {totalResults: response[0].length};
